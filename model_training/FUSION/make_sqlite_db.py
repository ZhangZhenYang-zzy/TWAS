...
class DB:
    def __init__(self, db_file):
        ...
        self.weights_fields = {"rsid", "gene", "weight", "ref_allele", "eff_allele"}
        self.extra_fields = {"gene", "genename", "pred.perf.R2", "n.snps.in.model",
    ...
    def insert_weights_row(self, row):
        ...
        self.c.execute("INSERT INTO weights VALUES (?, ?, ?, ?, ?)",
            (row['rsid'], row['gene'], row['weight'], row['ref_allele'],row['eff_allele']))
    ...
    def insert_extra_row(self, row):
        self.c.execute("INSERT INTO extra VALUES (?, ?, ?, ?, ?, ?)",(row['gene'],row['genename'],row['pred.perf.R2'],row['n.snps.in.model'],row['pred.perf.pval'], row['pred.perf.qval']))
    ...
    with open(source_file, 'r') as src:
        for k, line in enumerate(src):
            if k == 0:
                ...
                # tmp[2] = upconvert(tmp[2])
                # print(tmp)
                # yield dict(zip(header, tmp))
