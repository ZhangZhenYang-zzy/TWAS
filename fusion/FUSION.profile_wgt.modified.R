...
colnames = c( "id" , "nsnps" ,"nsamples", "hsq" , "hsq.se" , "hsq.pv" , paste(models,"adj.r2",sep='.') ,paste(models,"r2",sep='.') , paste(models,"pv",sep='.') )

mat.snps = matrix(nrow=N,ncol=1)
mat.mod.N = matrix(nrow=N,ncol=1)
mat.hsq = matrix(nrow=N,ncol=2)
mat.hsqpv = matrix(nrow=N,ncol=1)
mat.mod.adjrsq = matrix(nrow=N,ncol=length(models))
...
mat.hsq[i,] = hsq
mat.hsqpv[i] = hsq.pv
mat.mod.adjrsq[i,] = cv.performance[1,m]
mat.mod.rsq[i,] = 1-(1-cv.performance[1,m])*(N.tot-2)/(N.tot-1)
# N.tot
...
write.table( 
cbind( names , mat.snps ,mat.mod.N, format(mat.hsq,digits=2) , format(mat.hsqpv,digits=2) , format(round(mat.mod.adjrsq,3),digits=3), format(round(mat.mod.rsq,3),digits=3) , format(mat.mod.pv,digits=2) ),
quote=F,
col.names=colnames,
row.names=F,sep='\t')
