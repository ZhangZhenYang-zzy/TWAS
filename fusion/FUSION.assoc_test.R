...
option_list = list(
  ...
  make_option("--cpus", action="store", default=NA, type='integer',
              help="Threads used for TWAS."),
...
# results <- apply(data.frame("row"=c(1:nrow(wgtlist))),1, wgt_main)
library(parallel)
# 计算可用线程数，并设置并行使用线程数
no_cores <- opt$cpus
# 初始化
cl <- makeCluster(no_cores)
clusterExport(cl,c("opt","wgtlist","genos","allele.qc","sumstat","out.tbl","FAIL.ctr"),envir = environment())

results <-parLapply(cl, data.frame("row"=c(1:nrow(wgtlist)))$row, wgt_main)
# print(results)
out.tbl<- do.call('rbind',results)
stopCluster(cl)
...
# save(out.tbl, file = "out.tbl.RDat")
write.table( format( out.tbl[!is.na(out.tbl$PANEL),] , digits=3 ) , quote=F , row.names=F , sep='\t' , file=opt$out )
